...
parser.add_argument("--fileName", help="name of your training data file", default="*.czi")
...
files = glob.glob(os.path.join(args.dataPath, args.fileName))
files.sort()
imgs = np.array([np.squeeze(czifile.imread(file))[..., np.newaxis] for file in files])
...
print(pshape)
patches = N2V_DataGenerator().generate_patches_from_list(imgs[:1], shape=pshape, augment=(not args.noAugment))
